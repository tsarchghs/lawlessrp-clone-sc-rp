...
function initializeFurnitureCatalog()
    ...
    MySQL.query("SELECT COUNT(*) as count FROM furniture_catalog", {}, function(result)
        if result[1].count == 0 then
            -- Furniture categories: Living Room, Bedroom, Kitchen, Bathroom, Office, Outdoor, Decoration, Storage, Misc
            
            -- Living Room
            MySQL.query("INSERT INTO furniture_catalog (Name, Model, Category, Price, Level, TextureOptions, IsStorage, StorageSlots, Description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
function loadHouses()
    MySQL.query("SELECT * FROM advanced_houses", {}, function(houses)
        if houses then
...
function loadHouseFurniture(houseId)
    MySQL.query("SELECT * FROM house_furniture WHERE HouseID = @houseId", {
        ['@houseId'] = houseId
    }, function(furniture)
        if furniture then
...
function createHouse(exteriorX, exteriorY, exteriorZ, exteriorA, interiorId, price, name, description)
    MySQL.query("INSERT INTO advanced_houses (ExteriorX, ExteriorY, ExteriorZ, ExteriorA, InteriorID, Price, Name, Description) VALUES (@x, @y, @z, @a, @interiorId, @price, @name, @description)", {
        ['@x'] = exteriorX,
        ['@y'] = exteriorY,
        ['@z'] = exteriorZ,
        ['@a'] = exteriorA,
        ...
        if result.insertId then
            ...
            -- Load the house data
            MySQL.query("SELECT * FROM advanced_houses WHERE ID = @id", {
                ['@id'] = houseId
            }, function(houses)
...
function buyHouse(source, houseId)
    ...
    -- Update house owner
    MySQL.query("UPDATE advanced_houses SET OwnerID = @ownerId WHERE ID = @houseId", {
...
function sellHouse(source, houseId)
    ...
    -- Update house owner
    MySQL.query("UPDATE advanced_houses SET OwnerID = 0 WHERE ID = @houseId", {
        ['@houseId'] = houseId
    }, function(result)
        if result.affectedRows > 0 then
            -- Update local data
            ...
            MySQL.query("DELETE FROM house_roommates WHERE HouseID = @houseId", {
                ['@houseId'] = houseId
            })
...
function toggleHouseLock(source, houseId)
    ...
    if house.OwnerID ~= player.CharacterID then
        ...
        MySQL.query("SELECT * FROM house_roommates WHERE HouseID = @houseId AND CharacterID = @charId", {
            ['@houseId'] = houseId,
    ...
    function toggleLock()
        ...
        MySQL.query("UPDATE advanced_houses SET Locked = @locked WHERE ID = @houseId", {
            ['@locked'] = newLockState,
            ['@houseId'] = houseId
        }, function(result)
            if result.affectedRows > 0 then
...
function buyFurniture(source, furnitureId)
    ...
    -- Get furniture info
    MySQL.query("SELECT * FROM furniture_catalog WHERE ID = @id", {
        ['@id'] = furnitureId
    }, function(furniture)
...
function placeFurniture(source, posX, posY, posZ, rotX, rotY, rotZ, textureId)
    ...
    MySQL.query("INSERT INTO house_furniture (HouseID, FurnitureID, PosX, PosY, PosZ, RotX, RotY, RotZ, TextureID) VALUES (@houseId, @furnitureId, @posX, @posY, @posZ, @rotX, @rotY, @rotZ, @textureId)", {
        ['@houseId'] = houseId,
...
function updateFurniturePosition(source, furnitureId, posX, posY, posZ, rotX, rotY, rotZ)
    ...
    MySQL.query("UPDATE house_furniture SET PosX = @posX, PosY = @posY, PosZ = @posZ, RotX = @rotX, RotY = @rotY, RotZ = @rotZ WHERE ID = @id", {
        ['@posX'] = posX,
        ['@posY'] = posY,
        ['@posZ'] = posZ,
        ['@rotX'] = rotX,
...
function removeFurniture(source, furnitureId)
    ...
    MySQL.query("SELECT Price FROM furniture_catalog WHERE ID = @id", {
        ...
        MySQL.query("DELETE FROM house_furniture WHERE ID = @id", {
            ['@id'] = furnitureId
...
function addRoommate(source, targetId)
    ...
    MySQL.query("SELECT * FROM house_roommates WHERE HouseID = @houseId AND CharacterID = @charId", {
        ...
        -- Add roommate
        MySQL.query("INSERT INTO house_roommates (HouseID, CharacterID) VALUES (@houseId, @charId)", {
            ['@houseId'] = houseId,
            ['@charId'] = target.CharacterID
        }, function(result)
            if result.insertId then
...
function removeRoommate(source, targetId)
    ...
    -- Get player's houses
    MySQL.query("SELECT * FROM advanced_houses WHERE OwnerID = @ownerId", {
        ['@ownerId'] = player.CharacterID
    }, function(houses)
        ...
        for _, house in ipairs(houses) do
            ...
            MySQL.query("SELECT * FROM house_roommates WHERE HouseID = @houseId AND CharacterID = @charId", {
                ...
                if roommates and #roommates > 0 then
                    ...
                    -- Remove roommate
                    MySQL.query("DELETE FROM house_roommates WHERE HouseID = @houseId AND CharacterID = @charId", {
                        ['@houseId'] = house.ID,
                        ['@charId'] = target.CharacterID
                    }, function(result)
                        if result.affectedRows > 0 then
...
function accessFurnitureStorage(source, furnitureId)
    ...
    -- Get storage items
    MySQL.query("SELECT * FROM house_storage WHERE HouseID = @houseId", {
        ['@houseId'] = houseId
    }, function(items)
...
function storeItemInFurniture(source, furnitureId, itemName, quantity, data)
    ...
    MySQL.query("SELECT COUNT(*) as count FROM house_storage WHERE HouseID = @houseId", {
        ['@houseId'] = houseId
    }, function(result)
        ...
        -- Store item
        MySQL.query("INSERT INTO house_storage (HouseID, ItemName, Quantity, Data) VALUES (@houseId, @itemName, @quantity, @data)", {
            ['@houseId'] = houseId,
            ['@itemName'] = itemName,
            ['@quantity'] = quantity,
            ['@data'] = data or nil
...
function takeItemFromFurniture(source, furnitureId, storageItemId)
    ...
    -- Get storage item
    MySQL.query("SELECT * FROM house_storage WHERE ID = @id AND HouseID = @houseId", {
        ['@id'] = storageItemId,
        ['@houseId'] = houseId
    }, function(items)
        ...
        if added then
            ...
            MySQL.query("DELETE FROM house_storage WHERE ID = @id", {
                ['@id'] = storageItemId
            }, function(result)
                if result.affectedRows > 0 then
                    -- Update storage view
...
function setHouseDescription(source, description)
    ...
    MySQL.query("UPDATE advanced_houses SET Description = @description WHERE ID = @houseId", {
        ['@description'] = description,
        ['@houseId'] = houseId
    }, function(result)
        if result.affectedRows > 0 then
...
function setHouseName(source, name)
    ...
    -- Update house name
    MySQL.query("UPDATE advanced_houses SET Name = @name WHERE ID = @houseId", {
        ['@name'] = name,
        ['@houseId'] = houseId
    }, function(result)
        if result.affectedRows > 0 then
...
function setHouseForSale(source, price)
    ...
    MySQL.query("UPDATE advanced_houses SET ForSale = 1, Price = @price WHERE ID = @houseId", {
        ['@price'] = price,
        ['@houseId'] = houseId
    }, function(result)
        if result.affectedRows > 0 then
...
function cancelHouseSale(source)
    ...
    -- Update house sale status
    MySQL.query("UPDATE advanced_houses SET ForSale = 0 WHERE ID = @houseId", {
        ['@houseId'] = houseId
    }, function(result)
        if result.affectedRows > 0 then
            -- Update local data
...
function upgradeHouse(source)
    ...
    -- Upgrade house
    MySQL.query("UPDATE advanced_houses SET Level = Level + 1, MaxFurniture = MaxFurniture + 25, SecurityLevel = SecurityLevel + 1 WHERE ID = @houseId", {
        ['@houseId'] = houseId
    }, function(result)
        if result.affectedRows > 0 then
            -- Update local data
...
function payHouseMaintenance(source)
    ...
    MySQL.query("SELECT * FROM advanced_houses WHERE OwnerID = @ownerId", {
        ...
        -- Pay maintenance
        for _, house in ipairs(houses) do
            MySQL.query("UPDATE advanced_houses SET LastPaid = NOW() WHERE ID = @houseId", {
                ['@houseId'] = house.ID
            })
        end
...
function triggerHouseAlarm(houseId, intruderSource)
    ...
    -- Get house owner
    MySQL.query("SELECT * FROM characters WHERE ID = @ownerId", {
...
function checkHouseMaintenance()
    MySQL.query("SELECT * FROM advanced_houses WHERE OwnerID > 0 AND DATEDIFF(NOW(), LastPaid) > 7", {}, function(houses)
        if houses then
            for _, house in ipairs(houses) do
                ...
                MySQL.query("SELECT * FROM characters WHERE ID = @ownerId", {
                    ...
                    if owners and #owners > 0 then
                        ...
                        MySQL.query("SELECT DATEDIFF(NOW(), LastPaid) as days FROM advanced_houses WHERE ID = @houseId", {
                            ['@houseId'] = house.ID
                        }, function(result)
                            if result and result[1].days > 14 then
                                -- Repossess house
                                MySQL.query("UPDATE advanced_houses SET OwnerID = 0, ForSale = 1 WHERE ID = @houseId", {
                                    ['@houseId'] = house.ID
                                })
                                
                                -- Remove all furniture and storage
                                MySQL.query("DELETE FROM house_furniture WHERE HouseID = @houseId", {
                                    ['@houseId'] = house.ID
                                })
                                MySQL.query("DELETE FROM house_storage WHERE HouseID = @houseId", {
                                    ['@houseId'] = house.ID
                                })
                                MySQL.query("DELETE FROM house_roommates WHERE HouseID = @houseId", {
                                    ['@houseId'] = house.ID
