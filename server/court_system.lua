...
function initializeLegalForms()
    ...
    MySQL.query("SELECT COUNT(*) as count FROM legal_documents WHERE DocumentType = 'Template'", {}, function(result)
        if result[1].count == 0 then
            ...
            for _, template in ipairs(templates) do
                MySQL.query("INSERT INTO legal_documents (DocumentType, Title, Content, AuthorID, Status) VALUES (@type, @title, @content, 0, 'Template')", {
                    ['@type'] = template.type,
...
function fileCourtCase(source, defendantId, caseType, description, damagesAmount)
    ...
    -- File the case
    MySQL.query("INSERT INTO court_cases (CaseNumber, PlaintiffID, DefendantID, CaseType, Description, FilingFee, Damages) VALUES (@caseNumber, @plaintiffId, @defendantId, @caseType, @description, @filingFee, @damages)", {
...
function applyForLawyerLicense(source)
    ...
    MySQL.query("SELECT * FROM lawyer_licenses WHERE CharacterID = @charId", {
...
function completeBarExam(source, score)
    ...
    -- Issue license
    MySQL.query("INSERT INTO lawyer_licenses (CharacterID, LicenseNumber, BarExamScore, ExpiryDate) VALUES (@charId, @licenseNumber, @score, DATE_ADD(NOW(), INTERVAL 2 YEAR))", {
        ['@charId'] = player.CharacterID,
...
function hireLawyer(source, caseId, lawyerId, side)
    ...
    MySQL.query("UPDATE court_cases SET " .. columnName .. " = @lawyerId WHERE ID = @caseId", {
        ['@lawyerId'] = lawyerId,
        ['@caseId'] = caseId
    }, function(result)
...
function scheduleCourtHearing(source, caseId, judgeId, courtDate, courtTime)
    ...
    MySQL.query("UPDATE court_cases SET JudgeID = @judgeId, CourtDate = @courtDate, Status = 'Scheduled' WHERE ID = @caseId", {
        ['@judgeId'] = judgeId,
...
function startCourtSession(source, caseId)
    ...
    -- Start the session
    MySQL.query("UPDATE court_cases SET Status = 'In Session' WHERE ID = @caseId", {
        ['@caseId'] = caseId
    }, function(result)
        if result.affectedRows > 0 then
...
function recordCourtTranscript(caseId, speakerId, role, statement)
    MySQL.query("INSERT INTO court_transcripts (CaseID, SessionDate, SpeakerID, SpeakerRole, Statement) VALUES (@caseId, NOW(), @speakerId, @role, @statement)", {
        ['@caseId'] = caseId,
        ['@speakerId'] = speakerId,
        ['@role'] = role,
        ['@statement'] = statement
...
function closeCourtCase(source, caseId, verdict, damages)
    ...
    -- Close the case
    MySQL.query("UPDATE court_cases SET Status = 'Closed', Verdict = @verdict, Damages = @damages, ClosedDate = NOW() WHERE ID = @caseId", {
        ['@caseId'] = caseId,
        ['@verdict'] = verdict,
        ...
        if result.affectedRows > 0 then
            ...
            if case.LawyerPlaintiffID and LawyerLicenses[case.LawyerPlaintiffID] then
                ...
                MySQL.query("UPDATE lawyer_licenses SET CasesWon = @won, CasesLost = @lost WHERE CharacterID = @charId", {
...
function loadCourtSystem()
    -- Load court cases
    MySQL.query("SELECT * FROM court_cases WHERE Status != 'Closed'", {}, function(cases)
        if cases then
    ...
    MySQL.query("SELECT * FROM lawyer_licenses WHERE LicenseStatus = 'Active'", {}, function(licenses)
        if licenses then
